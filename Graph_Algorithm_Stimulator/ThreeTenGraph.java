...
class ThreeTenGraph implements Graph<GraphNode,GraphEdge>, UndirectedGraph<GraphNode,GraphEdge> {
    ...
    public ThreeTenGraph() {
        ...
        nodeList = (LinkedList<GraphNode>) new LinkedList();
        adjList = (LinkedList<Destination>[]) new LinkedList[MAX_NUMBER_OF_NODES];//MIGHT BE WRONG
    ...
    public Collection<GraphEdge> getEdges() {
        ...
        Collection<GraphEdge> xo = new LinkedList<>();
        LinkedList<Destination> ovo = new LinkedList<>();
        if(adjList.length==0){
            return null;
        }
        for(int i = 0; i < nodeList.size(); i++){
            if(adjList[nodeList.get(i).id]==null||adjList[nodeList.get(i).id].size()==0){
                ;
            }
            else{
                for(int j = 0; j < adjList[nodeList.get(i).id].size(); j++){
                    ovo.add(adjList[nodeList.get(i).id].get(j));
                }
            }
        }
        for(int i = 0; i < ovo.size(); i++){
            xo.add(ovo.get(i).edge);
        }
        return xo;
    }
    ...
    public Collection<GraphNode> getVertices() {
        ...
        //Hint: this method returns a Collection, look at the imports for
        //what collections you could return here.
        Collection<GraphNode> ovo = new LinkedList<>();
        if(nodeList.isEmpty()){
            return null;
        }
        for(int i = 0; i < nodeList.size(); i++){
            ovo.add(nodeList.get(i));
        }
        return ovo;
    }
    ...
    public int getEdgeCount() {
        ...
        int numofedges = 0;
        for(int i = 0; i< nodeList.size(); i++){
            if(adjList[nodeList.get(i).id]==null){
                ;
            }
            else{
                numofedges += adjList[nodeList.get(i).id].size();
            }
        }
        return numofedges/2;
    }
    ...
    public Collection<GraphNode> getNeighbors(GraphNode vertex) {
        //O(n) where n is the number of nodes in the graph
        Collection<GraphNode> neigh = new LinkedList<>();
        if(nodeList.contains(vertex)){
            //int loc = nodeList.indexOf(vertex);
            for(int i = 0; i < adjList[nodeList.get(nodeList.indexOf(vertex)).id].size(); i++){
                neigh.add(adjList[nodeList.get(nodeList.indexOf(vertex)).id].get(i).node);
            }
        }
        else{
            return null;
    ...
    public int getNeighborCount(GraphNode vertex) {
        ...
        return getNeighbors(vertex).size(); //DOUBLE CHECK W/ TA ABOUT THIS
    }
    ...
    public Collection<GraphEdge> getIncidentEdges(GraphNode vertex) {
        //O(n) where n is the number of nodes in the graph
        Collection<GraphEdge> listofedges = new LinkedList<>();
        if(nodeList.contains(vertex)){
            //int loc = nodeList.indexOf(vertex);
            for(int i = 0; i < adjList[nodeList.get(nodeList.indexOf(vertex)).id].size(); i++){
                listofedges.add(adjList[nodeList.get(nodeList.indexOf(vertex)).id].get(i).edge);
            }
        } //ASK TA IF U SHOULD THROW AN EXCEPTION IF VERTEX IS NOT IN THE GRAPH
        else{
            return null;
        }
        //return neigh;
    ...
    public Pair<GraphNode> getEndpoints(GraphEdge edge) {
        ...
        for(int i = 0; i< nodeList.size(); i++){
            if(adjList[nodeList.get(i).id]!=null){
                for(int j = 0; j<adjList[nodeList.get(i).id].size(); j++){
                    if(adjList[nodeList.get(i).id].get(j).edge.equals(edge)){
                        one = nodeList.get(i);
                        two = adjList[nodeList.get(i).id].get(j).node;
                        Pair<GraphNode> pairs = new Pair<>(one, two);
                        return pairs;
                    }
                }
            }
    ...
    public GraphEdge findEdge(GraphNode v1, GraphNode v2) {
        ...
        Object an;
        GraphEdge link = null;
        LinkedList<Destination> node1 = new LinkedList();
        LinkedList<Destination> node2 = new LinkedList();
        //int index1 = 0;
        //int index2 = 0;
        for(int i = 0; i<adjList[v1.id].size(); i++){
            if(adjList[v1.id].get(i).node.equals(v2)){
                link = adjList[v1.id].get(i).edge;
                return link;
            }
        }
        return null;
    ...
    public boolean addEdge(GraphEdge e, GraphNode v1, GraphNode v2) {
        ...
        for(int i = 0; i < nodeList.size(); i++){
            if(adjList[nodeList.get(i).id]!=null){
                if(adjList[nodeList.get(i).id].contains(e)){
                    return false;
                }
            }
        }
        if(!nodeList.contains(v1)||!nodeList.contains(v2)){
            throw new IllegalArgumentException("The vertices don't exist in this graph");
        }
        if(v1.id>v2.id){
            max = v1.id;
        }
        ...
        if(adjList[v1.id]==null){
            adjList[v1.id] = new LinkedList<>();
            adjList[v1.id].add(new Destination(v2, e));
        }
        else{
            adjList[v1.id].add(new Destination(v2, e));
        }
        if(adjList[v2.id]==null){
            adjList[v2.id] = new LinkedList<>();
    ...
    public boolean addVertex(GraphNode vertex) {
        ...
        if(vertex==null||nodeList.contains(vertex)){
            return false;
        }
        nodeList.add(vertex);
    ...
    public boolean removeEdge(GraphEdge edge) {
        ...
        for(int i = 0; i < nodeList.size(); i++){
            for(int j = 0; j < adjList[nodeList.get(i).id].size(); j++){
                if(adjList[nodeList.get(i).id].get(j).edge.equals(edge)){
                    //adjList[nodeList.get(i).id].get(j).edge.id
                    found = true;
                    second = adjList[nodeList.get(i).id].get(j).node;
                    adjList[nodeList.get(i).id].remove(j);
                    //rem = true;
                }
            }
            if(found==true){
        ...
        for(int k = 0; k < adjList[second.id].size(); k++){
            if(adjList[second.id].get(k).edge.equals(edge)){
                adjList[second.id].remove(k);
                return true;
            }
        }
        return false;
    ...
    public boolean removeVertex(GraphNode vertex) {
        ...
        //the adjacency list works for nodes that have no edges vs. nodes
        //that aren't in the graph
        if(!nodeList.contains(vertex)||vertex==null){
            return false;
        }
        nodeList.remove(vertex);
        for(int i = adjList[vertex.id].size()-1; i>=0; i--){
            adjList[vertex.id].remove(i);
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        GraphEdge[] edges = { new GraphEdge(0), new GraphEdge(1), new GraphEdge(2), new GraphEdge(3), new GraphEdge(4), new GraphEdge(5)};
        ...
        //System.out.println(graph.nodeList.size());
        if(graph.getVertexCount() == 10 && graph.getEdgeCount() == 6) {
            System.out.println("Yay 1");
        }
        //System.out.println(graph.containsVertex(new GraphNode(0)));
        if(graph.containsVertex(new GraphNode(0)) && graph.containsEdge(new GraphEdge(3))) {
            System.out.println("Yay 2");
        }
        //System.out.println("List of vertices: " + graph.getVertices());
        //System.out.println("List of edges: " + graph.getEdges());
        //System.out.println("Number of edges: " + graph.getEdgeCount());
        //Collection<GraphEdge> jesus = graph.getEdges();
